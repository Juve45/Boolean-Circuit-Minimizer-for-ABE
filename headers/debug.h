...
// template<class T> void debug_out(T t) { std::cerr << ' ' << t; }
template<class Head, class... Tail> void debug_out(Head H, Tail... T) { std::cerr << ' ' << H; debug_out<Head>(T...); }
// template<class T> using ordered_set = tree<T, null_type, less<T>, rb_tree_tag, tree_order_statistics_node_update>; // ordered_set<ynt> s; // order_of_key(value)
